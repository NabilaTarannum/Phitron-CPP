...
Node *reverseNonRecursive(Node *&head) // O(n)
{
...
Node *reverseRecursive(Node *&head) // O(n)
{
    ...
    Node *temp = head->Next;
    head->Next = NULL;
    Node *newHead = reverseRecursive(temp);
    temp->Next = head;
    return newHead;
}

int findMid(Node *&head) // O(n)
{
    if (head == NULL)
    {
        return -1;
...
void makeCycle(Node *&head, int position) // O(n)
{
...
void removeCycle(Node *&head) // O(n)
{
    Node *slow = head, *fast = head;
    do {
        slow = slow->Next;
...
int main()
{
    ...
    cout << "Choice 01 : Insertion at head\n" <<
			...
			"Choice 04 : Search a specific value (Unique list)\n" <<
			"Choice 05 : Search a specific value (Duplicate enabled list)\n" <<
			...
			"Choice 12 : Deletion by Value (duplicate enabled list)\n" <<
            "Choice 13 : Reversal of List Non-Recursive\n" <<
            "Choice 14 : Reversal of List Recursive\n" <<
            "Choice 15 : Finding the Mid (Slow-Fast pointer method)\n" <<
            "Choice 16 : Make a Cycle at k position\n" <<
            "Choice 17 : Detect Cycle in the list\n" <<
            "Choice 18 : Remove Cycle from the list (if it exists)\n" <<
			"Choice 00 : Exit\n\n" <<
			"Choice ?? : ";
	cin >> choice;
	while (choice != 0)
	{
        ...
        case 15:
            ...
            break;
        case 16:
            cout << "Enter the desired position to create the cycle: ";
            cin >> position;
            makeCycle(head, position);
            break;
        case 17:
            cycleStatas = detectCycle(head);
            if (cycleStatas == true)
